...
AddFloorWindow::AddFloorWindow(wxWindow* parent, std::vector<std::string> names, int num_added)
    ...
    // num_added is used to know how many floors there are total, needed in the get_floor_neighbors function
    num_added_ = num_added;

    // Main sizer for the window
    wxBoxSizer* sizer = new wxBoxSizer(wxVERTICAL);

    // Throw a wxID_CANCEL when the window closes, rather than the default wx_ID_OK
